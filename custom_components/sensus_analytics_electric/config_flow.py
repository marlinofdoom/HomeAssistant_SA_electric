...
class SensusAnalyticsConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Sensus Analytics Integration (Electric)."""
    ...
    async def async_step_user(self, user_input=None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            unique_id = f"{user_input[CONF_ACCOUNT_NUMBER]}_{user_input[CONF_ELECTRIC_METER_NUMBER]}"
            ...
            if valid:
                return self.async_create_entry(title="Sensus Analytics (Electric)", data=user_input)
            errors["base"] = "auth"
        ...
        data_schema = vol.Schema(
            {
                ...
                vol.Optional("electric_solar_credit_price", default=0.0872): cv.positive_float,
        ...
        data_schema = vol.Schema(
            {
                ...
                vol.Required(
                    ...
                    default=current_data.get("electric_commodity_price", 0.141300),
                ): cv.positive_float,
                vol.Optional(
                    "electric_solar_credit_price",
                    default=current_data.get("electric_solar_credit_price", 0.0872000),
                ): cv.positive_float,
                vol.Required(
                    "electric_service_fee",
                    default=current_data.get("electric_service_fee", 12.60),
                ): cv.positive_float,
            }
        )
